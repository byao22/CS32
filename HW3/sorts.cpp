      ...
      // destruction time.
    {
     vector<Sensor> auxSensors(sensors);
      ...
      //        to point to the corresponding Sensor in auxSensors.
        vector<Sensor*> SensorPtrs; // new vector
        for(vector<Sensor>::iterator i = auxSensors.begin(); i != auxSensors.end(); i++){
            SensorPtrs.push_back(&*i);
        }
    
      // TODO:  Sort the vector of pointers using the STL sort algorithm
      //        with compareSensorPtr as the ordering relationship.
        sort(SensorPtrs.begin(), SensorPtrs.end(), compareSensorPtr); // use earlier defined function of compareSensorPtr to sort from beginning to end
      ...
      //        in sensors with the Sensors from auxSensors in the correct order.
        for (int i = 0; i < auxSensors.size(); i++)
            sensors[i] = *SensorPtrs[i]; // parse through each element
