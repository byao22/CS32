...
{
    m_age++;
    m_arena->thePast().markIt(m_row, m_col);
    // make this identify as a placement 
}
...
void Player::move(int dir)
{
    ...
    switch (dir)
    {
        case UP:     if (m_row > 1)               m_row--; m_arena->thePast().markIt(m_row, m_col);break;
        case DOWN:   if (m_row < m_arena->rows()) m_row++; m_arena->thePast().markIt(m_row, m_col); break;
        case LEFT:   if (m_col > 1)               m_col--; m_arena->thePast().markIt(m_row, m_col); break;
        case RIGHT:  if (m_col < m_arena->cols()) m_col++; m_arena->thePast().markIt(m_row, m_col); break;
    } // where we started to get variables for grid 
}
